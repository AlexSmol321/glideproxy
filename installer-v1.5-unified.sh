...
# Версия: 1.5-unified  (Node.js 20 LTS, dual cert engine, auto‑renew)
# Автор: Proxy Deployment System
#
# Режимы сертификатов
#   CERT_MODE=letsencrypt (по умолчанию) — автоматический Certbot
#   CERT_MODE=timeweb                   — коммерческий Timeweb PRO
#       требуются TIMEWEB_TOKEN  и TIMEWEB_CERT_ID
#
# Параметры можно передать  export VAR=value  или ввести интерактивно.
# ==================================================================
set -euo pipefail
...
[[ $EUID -ne 0 ]] && die "Запустите скрипт через sudo"

# ---------- helper ----------
read_var(){ local v=$1 p=$2 d=$3 s=${4:-0}; [[ -n "${!v:-}" ]] && return
  if (( s )); then read -s -p "$p" $v && echo; else read -p "$p" $v; fi
  [[ -z "${!v}" ]] && eval "$v=$d"
}

# ---------- input ----------
read_var PROXY_DOMAIN  "Proxy domain                : " ""
read_var TARGET_DOMAIN "Target domain               : " ""
read_var SSL_EMAIL     "Email Let'sEncrypt          : " ""
read_var PROJECT_NAME  "Project name     [my-proxy] : " "my-proxy"
read_var NODE_PORT     "Node port        [3000]     : " "3000"
read_var MAX_MEMORY    "PM2 max memory   [512M]     : " "512M"

CERT_MODE=${CERT_MODE:-letsencrypt}
if [[ $CERT_MODE == timeweb ]]; then
  read_var TIMEWEB_TOKEN    "Timeweb API token (hidden) : " "" 1
  read_var TIMEWEB_CERT_ID  "Timeweb Cert ID            : " ""
fi
...
ok "Base stack ready (Node 20 + PM2)"
...
app.get('/health/detailed', (_req,res)=>res.json({status:'ok', target:TARGET, uptime:process.uptime()}));
...
if [[ $CERT_MODE == letsencrypt ]]; then
  info "Obtaining Let's Encrypt cert..."
...
# ---------- npm & pm2 ----------
cd "$PROJECT_DIR" && npm ci --production -s
...
# ---------- renew script + cron ----------
if [[ $CERT_MODE == timeweb ]]; then
  info "Creating Timeweb auto-renew script..."
...
curl -sf "${auth[@]}" -H "Content-Type: application/json" -X POST      "$api/ssl-certificates/$TIMEWEB_CERT_ID/renew" -d '{}' >/dev/null
for i in {1..12}; do
  status=$(curl -sf "${auth[@]}" "$api/ssl-certificates/$TIMEWEB_CERT_ID" | jq -r .status)
  [[ $status == issued ]] && break; sleep 60
done
tmp=$(mktemp -d); trap 'rm -r $tmp' EXIT
curl -sf "${auth[@]}" -H "Accept: application/zip" "$api/ssl-certificates/$TIMEWEB_CERT_ID/download" -o "$tmp/c.zip"
unzip -qo "$tmp/c.zip" -d "$tmp"
...
echo "$(date) renewed OK"
RENEW
  chmod 700 "$PROJECT_DIR/scripts/renew-timeweb.sh"
  echo "0 4 4 * * root $PROJECT_DIR/scripts/renew-timeweb.sh >> /var/log/timeweb-renew.log 2>&1"      > /etc/cron.d/timeweb-renew
fi

ok "Install complete → https://$PROXY_DOMAIN/"
